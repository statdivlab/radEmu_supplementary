...
# parallelize 504 trials for 14 settings across 882 tasks
nsim <- 504
sim_per_script <- 8
script_per_setting <- 63
seeds <- ((batch - 1) %% script_per_setting) * sim_per_script + 1:sim_per_script
curr_setting <- floor((batch - 1) / script_per_setting) + 1
settings_rerun <- c(2, 9, 20, 27, 38, 45, 56, 63, 74, 81, 92, 99, 110, 117)
setting <- settings_rerun[curr_setting]
...
sim_function <- function(sim_no, seeds) {
  ...
  for (sim in 1:length(seeds)) {
    print(paste0("setting: ", sim_no, ", seed: ", sim))
    set.seed(seeds[sim])
...
saveRDS(results, paste0(sim_dir_name, "/rerun_setting", setting, "_first_seed", seeds[1], ".rds"))